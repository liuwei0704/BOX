...
class Spider(BaseSpider):
    ...
    def _parse_detail(self, html, vid):
        ...
        if play_url:
            ...
            if play_url.startswith("http") and (".m3u8" in play_url or ".mp4" in play_url):
                ...
                # 哈希值或其他非直链格式，返回原始哈希值，在playerContent中处理
                vod["vod_play_url"] = f"播放${play_url}"
                vod["vod_play_from"] = "哈希"
        else:
    ...
    def playerContent(self, flag, id, vipFlags=None):
        try:
            ...
            # 如果是哈希值（32位十六进制），交给系统嗅探
            if id and re.match(r'^[0-9a-f]{32}$', id, re.I):
                self.log(f"playerContent: hash value detected, fallback to sniff: {id}")
                # 构造完整的播放页URL
