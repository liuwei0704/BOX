...
class Spider(BaseSpider):
    ...
    def searchContent(self, key, quick, pg='1'):
        page = int(pg) if pg else 1
        # 搜索URL格式：/vodsearch/-------------.html?wd=关键词
        url = f'{self.host}/vodsearch/-------------.html?wd={quote(key)}'
        self._log(f'搜索URL: {url}')
        html = self._fetch(url, referer=self.host)
        items = self._parse_list(html) if html else []
        # 检查是否有下一页
        has_next = False
        if html:
            next_link = re.search(r'<a[^>]+href="[^"]*page=(\d+)"[^>]*>下一页</a>', html)
            if next_link:
                has_next = True
            if not has_next:
                next_link = re.search(r'<a[^>]+>(?:下页|下一页|&gt;)[^<]*</a>', html)
                if next_link:
                    href_match = re.search(r'href="([^"]*page=(\d+)[^"]*)"', next_link.group(0))
                    if href_match:
                        has_next = True
