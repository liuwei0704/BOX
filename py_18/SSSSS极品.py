# coding: utf-8
# SSSSS极品 影视爬虫 - MacCMS 标准站
# 站点: https://ssswave.sssssj12.icu/123/
...
class Spider(BaseSpider):
    def __init__(self):
        self.host = "https://ssswave.sssssj12.icu"
        self.base_path = "/123"
        self.site_name = "SSSSS极品"
        self.classes = [
            {"type_id": "252", "type_name": "偷拍偷窥"},
    ...
    def homeVideoContent(self):
        html = self._fetch_html(self.host + self.base_path + "/")
    ...
    def recommendContent(self, ids, pg):
        ...
        try:
            ...
            ul_start = html.find('<ul class="stui-vodlist clearfix">', pos)
            if ul_start == -1:
                return {"list": []}
            ul_end = html.find('</ul>', ul_start + 10)
            if ul_end == -1:
                return {"list": []}
            
            ul_html = html[ul_start:ul_end + 5]
            items = self._parse_video_list(ul_html)
    ...
    def _parse_video_list(self, html):
        ...
        link_pattern = r'<a[^>]*href="[^"]*/detail/id/(\d+)/[^"]*"[^>]*title="([^"]*)"'
        matches = re.findall(link_pattern, html)
        
        if not matches:
            link_pattern2 = r'<a[^>]*href="/123/index.php/vod/detail/id/(\d+).html"[^>]*title="([^"]*)"'
