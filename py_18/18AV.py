...
class Spider(Spider):
    ...
    def _build_tvbox_url(self, play_url, referer):
        ...
        # 先请求一次播放页，获取最新的 Cookie（包含 PHPSESSID）
        try:
            test_res = self.session.get(referer, headers={"User-Agent": self.ua}, verify=False, timeout=8)
            # 更新 cookies
        except:
            pass
        ...
        # 构建 Cookie 字符串（标准格式，用 ; 分隔）
        cookie_str = "; ".join([f"{k}={v}" for k, v in cookies.items()])
        
        # 构建 header 字符串（TVBox 格式：key@value 用 && 分隔）
        header_parts = [
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if id.startswith("iframe_1080@@") or id.startswith("iframe_720@@"):
            url = id.split("@@", 1)[1]
            # 使用 parse:1 让 TVBox 用 WebView 加载播放页面
            # 页面中的 HLS.js 播放器会自动处理 AES-128 解密和 JS 分片
        ...
        if id.startswith("direct@@"): 
            return {"parse": 0, "url": id.replace("direct@@", "", 1), "header": {"User-Agent": self.ua, "Referer": self.base_url + "/zh/"}}
    ...
    def localProxy(self, param):
        """本地代理：处理 m3u8、key.bin 和 .js 分片请求"""
        url = param.get("url", "")
        if not url:
        ...
        if "key.bin" in url or "aavv.vv" in url:
            try:
                resp = self.session.get(url, headers={"Referer": self.base_url + "/", "User-Agent": self.ua}, verify=False, timeout=10)
        ...
        # 处理 .js 分片请求（伪装成 TS）
        if ".js" in url and ("_000" in url or "_001" in url or "_002" in url):
            try:
                resp = self.session.get(url, headers={"Referer": self.base_url + "/", "User-Agent": self.ua}, verify=False, timeout=10)
                if resp.status_code == 200:
                    # 返回 TS 格式的 Content-Type
                    return [200, "video/MP2T", resp.content]
            except Exception as e:
                self.log({"action": "localProxy_js_fail", "error": str(e)})
            return [404, "text/plain", "Segment not found"]

        # 处理 m3u8 请求：重写内容，将 .js 替换为 .ts
        if ".m3u8" in url:
            try:
                resp = self.session.get(url, headers={"Referer": self.base_url + "/", "User-Agent": self.ua}, verify=False, timeout=10)
                if resp.status_code == 200:
                    ...
                    for line in lines:
                        ...
                        if line.startswith('#'):
                            ...
                            # 将 .js 分片改为 .ts（但实际请求时仍会转到上面的 .js 处理）
                            if '.js' in line:
                                line = line.replace('.js', '.ts')
