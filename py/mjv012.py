...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        if real_id:
            url_1080 = self._fix_url(f"/js/player/play.php?numresolution=1080&id={real_id}")
            url_720 = self._fix_url(f"/js/player/play.php?numresolution=720&id={real_id}")
            return {"list": [{"vod_id": raw, "vod_name": vod_name, "vod_pic": cover, "vod_content": vod_content, "vod_play_from": "1080P蓝光$$$720P高清", "vod_play_url": f"播放$iframe_1080@@{url_1080}$$$播放$iframe_720@@{url_720}"}]}

        m3u8_list = re.findall(r'(https?://[^"\'\s<>,\\]+?\.m3u8(?:\?[^"\'\s<>]*)?)', html_text, re.I)
        if m3u8_list:
            return {"list": [{"vod_id": raw, "vod_name": vod_name, "vod_pic": cover, "vod_content": vod_content, "vod_play_from": "原生直链", "vod_play_url": f"播放$direct@@{self._fix_url(m3u8_list[-1])}"}]}
    ...
    def _build_tvbox_url(self, play_url, referer):
        ...
        if 'YES_Eighteen' not in cookies: cookies['YES_Eighteen'] = 'IamOverEighteenYearsOld'
        if 'javascript_cookie_AgreeTOS_save1' not in cookies: cookies['javascript_cookie_AgreeTOS_save1'] = 'javascript_cookie_AgreeTOS_save2'
        ...
        headers = {
            ...
            "User-Agent": self.ua.replace(";", "；；"),
            "Referer": referer
        }
        
        ext_str = "&&".join([f"{k}@{v}" for k, v in headers.items()])
        return f"{play_url};{{{ext_str}}}"

    def playerContent(self, flag, id, vipFlags):
        if id.startswith("iframe_1080@@") or id.startswith("iframe_720@@"):
            is_1080 = id.startswith("iframe_1080@@")
            url = id.split("@@", 1)[1]
            try:
                res = self.session.get(url, headers={"User-Agent": self.ua, "Referer": url}, verify=False, timeout=10)
                if res.status_code == 200:
                    sources = []
                    # 👑 恢复正确的正则组解包逻辑，防止代码崩溃
                    blocks = re.findall(r"\{(src:\s*['\"][^'\"]+['\"].*?size:\s*\d+)\}", res.text, re.I)
                    if not blocks: blocks = re.findall(r"\{[^}]*src:\s*['\"][^'\"]+['\"][^}]*\}", res.text, re.I)

                    for block in blocks:
                        src_m = re.search(r"src:\s*['\"]([^'\"]+\.(?:m3u8|mp4)[^'\"]*)['\"]", block, re.I)
                        size_m = re.search(r"size:\s*(\d+)", block, re.I)
                        if src_m and size_m:
                            sources.append((src_m.group(1), size_m.group(1)))
                            
                    final_url = ""
                    if sources:
                        for src, size in sources:
                            if is_1080 and size == '1080': final_url = src
                            elif not is_1080 and size == '720': final_url = src
                        if not final_url: final_url = sources[-1][0]
                        
                    if not final_url:
                        m3u8_list = re.findall(r"src:\s*['\"](https?://[^'\"]+\.m3u8[^'\"]*)['\"]", res.text, re.I) or re.findall(r'(https?://[^"\'\s<>,\\]+?\.m3u8(?:\?[^"\'\s<>]*)?)', res.text, re.I)
                        if m3u8_list: final_url = m3u8_list[-1] if is_1080 else m3u8_list[0]
                    
                    if final_url:
                        formatted_url = self._build_tvbox_url(final_url.replace("\\/", "/"), url)
                        return {"parse": 0, "url": formatted_url, "header": {"User-Agent": self.ua, "Referer": url}}
            except: pass
        ...
        if id.startswith("direct@@"): return {"parse": 0, "url": self._build_tvbox_url(id.replace("direct@@", "", 1), self.base_url + "/zh/"), "header": {"User-Agent": self.ua, "Referer": self.base_url + "/zh/"}}
