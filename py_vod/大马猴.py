...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if isinstance(j, dict):
            # 尝试多种可能的字段名
            pc = j.get('pageCount') or j.get('pagecount') or j.get('total_page') or j.get('totalPage')
            if pc:
                try:
                    pagecount = int(pc)
                except:
                    pagecount = cur_page + 1
            else:
                # 如果没有 pageCount，尝试从 total 计算
                total_raw = j.get('total') or j.get('totalCount') or j.get('total_count') or 0
                if total_raw:
                    try:
                        total_int = int(total_raw)
                        limit_raw = j.get('limit') or j.get('pageSize') or 20
                        try:
                            limit_int = int(limit_raw)
                        except:
                            limit_int = 20
                        pagecount = (total_int + limit_int - 1) // limit_int
                        if pagecount < 1:
                            pagecount = 1
                    except:
                        pagecount = cur_page + 1
