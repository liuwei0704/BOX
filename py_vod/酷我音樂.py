...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        if rid.startswith('song_'):
            return self._song_detail_content(rid[5:])
    ...
    def _song_detail_content(self, rid):
        result = {}
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
            'Referer': 'http://www.kuwo.cn/'
        ...
        try:
            url = f"http://wapi.kuwo.cn/api/www/music/musicInfo?mid={rid}"
            data = requests.get(url, headers=headers, timeout=10).json().get('data', {})
            name = data.get('name', '')
            artist = data.get('artist', '')
            album = data.get('album', '')
            play_name = f"{name} - {artist}" if artist else name
            vod = {
                "vod_id": f"song_{rid}",
                "vod_name": play_name,
                "vod_pic": data.get('pic') or data.get('albumpic', ''),
                "vod_content": album,
                "vod_remarks": artist,
                "vod_actor": artist,
                "vod_play_from": "酷我音乐",
                "vod_play_url": f"{name}${rid}"
            }
            result['list'] = [vod]
        except Exception as e:
            result['list'] = [{
...
"""
    ...
    def searchContent(self, key, quick, pg=1):
        ...
        videos = []
        for search_type in ('artist', 'music'):
            url = f"http://search.kuwo.cn/r.s?client=kt&pn={page_num}&rn=30&all={wd}&vipver=1&ft={search_type}&encoding=utf8&rformat=json&mobi=1"
            try:
                data = requests.get(url, headers=headers, timeout=10).json()
                for item in data.get('abslist', []):
                    if search_type == 'artist':
                        artist_id = item.get('ARTISTID') or item.get('DC_TARGETID', '')
                        ...
                        video = {
                            "vod_id": str(artist_id),
                            "vod_name": item.get('ARTIST', ''),
                            "vod_pic": pic,
                            "vod_remarks": f"歌曲 :  {item.get('SONGNUM', 0)}首"
                        }
                    else:
                        song_id = item.get('DC_TARGETID') or item.get('MUSICRID', '').replace('MUSIC_', '')
                        pic = item.get('hts_MVPIC') or item.get('web_albumpic_short', '')
                        video = {
                            "vod_id": f"song_{song_id}",
                            "vod_name": item.get('NAME') or item.get('SONGNAME', ''),
                            "vod_pic": pic,
                            "vod_remarks": item.get('ARTIST', '')
                        }
                    videos.append(video)
            except Exception:
                continue
